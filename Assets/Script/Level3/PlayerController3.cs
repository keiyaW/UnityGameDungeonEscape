...
public class PlayerController3 : MonoBehaviour
{
    ...
    public GameObject lifeItem2;
    public GameObject lifeItem3;
    public GameObject lifeItem4;
    ...
    public void OnCollisionEnter2D(Collision2D coll)
    {
        ...
        if (name.Contains("Obstacle"))
        {
            ...
            Physics2D.IgnoreCollision(lifeItem2.GetComponent<Collider2D>(), GetComponent<Collider2D>(), false);
            Physics2D.IgnoreCollision(lifeItem3.GetComponent<Collider2D>(), GetComponent<Collider2D>(), false);
            Physics2D.IgnoreCollision(lifeItem4.GetComponent<Collider2D>(), GetComponent<Collider2D>(), false);
            if (!blink)
            {
                sfx.PlayOneShot(hit);
                blink = true;
    ...
    public void OnCollisionExit2D(Collision2D coll)
    {
        ...
        else if(name.Contains("BoxObject") && (transform.position.y > coll.gameObject.transform.position.y + 1.5))
        {
            canStep = false;
            steps.Stop();
        }
